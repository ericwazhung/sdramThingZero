...
ifdef AVRDUDE_MY_FTDI
AVRDUDE_PROGRAMMER = myFT2232
AVRDUDE_CONFIG_FILE = $(COMDIR)/_make/my_avrdude.conf
endif
...
ifdef AVRDUDE_CONFIG_FILE
AVRDUDE = avrdude -c $(AVRDUDE_PROGRAMMER) -p$(MCU) -C $(AVRDUDE_CONFIG_FILE)
else
...
#a/o Debian Jessie, and sdramThingZero...
# Apparently avrdude has a failure upon read (usb-tiny)
# But the output-file is created
# So instead of doing eeprom-read only if flash-read is OK, do both anyways
.PHONY: read
read:
	@mkdir -p $(BACKUP_DIR)
...
#.PHONY: eepromOnly
# a/o 6-2-16, revisiting MiniQ for PBG4 trade...
#  shopt isn't working for some reason... which is weird because it *is* in
#  Bash... so make's running a different shell, maybe?
#eepromOnly:
#	@shopt -s extglob ; \
#		sizes="`avr-size $(TARGET).eep`" ; \
#			 s1="$${sizes%+([[:blank:]])"$(TARGET).eep"}" ; \
...
#			 s5="$${s4%%+([[:blank:]])*}" ; \
#			 if [ "$$s5" != "0" ] ; \
# ... So, instead, use SystemV output, which shows a total at the bottom
#     (duh).
# ... This has been tested with an empty .eep file, as well as a non-empty
# one, copied over from an old project. a/o LCDdirectLVDS/150
# HOWEVER: Apparently the eeprom isn't being saved during chip-erase, so
# needs to be written each time. The .eep file was stolen from
# sdramThing3.0/14... there may've been an option (in the makefile?) for
# telling it to save the contents. No, a fuse, right...? YES A FUSE
# So, to test the third-case, I used 'make eepromOnly' and it worked.
.PHONY: eepromOnly
eepromOnly:
	@sizes="`avr-size -A $(TARGET).eep`" ; \
...
echo "################################################################################" ; \
	  		...
	  		echo "...Nothing to write in the eeprom..." ; \
		fi
...
# * /home/meh/_avrProjects/sdramThingZero/21-gittifying/_commonCode_localized/_make/avrCommon.mk
# *
