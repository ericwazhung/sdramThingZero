...
#reallyCommon3.mk++
#
# reallyCommon3.mk is the third version of the "reallyCommon*.mk" series
#  which follows after the "common.mk" series, which was AVR-specific
#  (reallyCommon*.mk is NOT specific to ANY architecture!)
#
# This handles quite a bit of stuff that is handy in most projects'
#  makefiles, including (at the very-least):
#    'make [all]'
#    'make run'
#    'make clean'
#     etc.
#
# It also handles its own scheme of things, including:
#    'make localize'
#    'make delocalize'
#     and more.
#
# Specifically: it handles working with "commonCode" and "commonThings"
#  "commonCode" is generally considered the entire set of individual
#  "commonThings" that can be useful to a large number of otherwise
#   unrelated projects.
#
#  commonCode is similar, in a way, to a shared-library folder on a PC
#   but, whereas shared-libraries are already compiled and ready-to-run
#   commonCode is code that must be compiled when needed.
#   (e.g. when compiling a project that makes use of it)
#   THUS: commonCode can be architecture-independent, which is especially
#   handy when cross-compiling and/or working with multiple architectures.
#   It's also handy for e.g. *smallish* pieces of code that could easily be
#   compiled directly into every application that uses it...
#   AND one of its larger-benefits is that code-revisions and bug-fixes can
#   then trickle-down to older projects (when recompiled) without having to
#   copy-over the newer version of the bug-fixed "commonThings"
#   Likewise, it's quite-handy when developing two projects side-by-side
#   that happen to share bits of code that may be modified as-you-go.
#
#
# VERSION LOG 
#  (has been very-poorly maintained up to '3++' and may likely
#   be out of order):
#
# 3+=2 - Adding end-warning regarding -Wundefined...
END_WARNINGS += ""
END_WARNINGS += "HEY!!! DO NOT TRUST -Wuninitialized or -Werror=uninitialized!"
END_WARNINGS += "SERIOUSLY! int a; if(b==3) a=3; return a; NO JOKE."
END_WARNINGS += ""
END_WARNINGS += "TODO: compare rc3.rec with reallyCommon3.mk"
#       ALSO: TODO: there was an unsaved swap file!
#        It's "rc3.rec"
#        NEEDS TO BE COMPARED.
#        (Though, this has been working for a while)
# 3++ - ARBITRARILY calling this version '3++'
#       I have not been keeping version-info with reallyCommon[123].mk
#       So, let's start.
#       THIS VERSION, a/o heartbeat 2.05
#         Removal of 'make tarball' it's long-since been forgotten
#          as 'make localize' came into being.
#         Extracting COMREL from COMDIR, so that they needn't both be
#          explicitly-defined in the project...
#          (Rather, we only need CENTRAL_COMDIR and LOCAL_COMDIR)
#         THIS SHOULD BE backwards-compatible with reallyCommon3.mk (NOT++)
#         TODO: REVISE Localization-block
#
# 3??
#     Changing over from reallyCommon2 to reallyCommon3:
#     COMDIR in the makefile should now be CENTRAL_COMDIR, etc.
#     This change-over was quite a mess of a make localize then changes,
#     then make delocalize... a/o heartbeat2.00-githubbing[+*evisions]
#
#  ??
...
#      Localization Block to go in makefile:
#      Add to makefile: (Uncomment indented commented lines)
#      (If using an AVR, definitely look into copying a code-block from 
#       there, as well, regarding PRINTF and STDIO and code-size)

# THIS IS OLD... But maybe not older than some of the pieced-together notes
# above (a/o v3++)
#
...
#   TODone: Figure that out...
#
#	Funny Note: I suppose this is getting closer to svn or cvs...?
#   TODone: Might also be handy to make changes to local commonCode and
...
#     probably: test $(MCU) here, and include avrCommon.mk (TODone)
#
# OLD NOTE:
# Sample makefile 
...
### END OF VERSION LOG ###
# (Though, since it was poorly-maintained, various version-notes may exist
#  throughout...)
...
### THIS HAS BEEN REVISED a/o 3++
#   THIS NOTE SHOULD BE CHANGED (TODO)
#
...
#COMDIR (from either LOCAL_COMDIR or CENTRAL_COMDIR) must NOT have a 
# trailing '/'
# 'dir' will leave the trailing '/'
#  but COMREL should not contain it, thus patsubst
#This ifndef is only for backwards-compatibility...
# Try to remove COMREL from project makefiles.
ifndef COMREL
COMREL = $(patsubst %/,%,$(dir $(COMDIR)))
endif
...
#a/o reallyCommon3.mk, this has been disabled, as has make tarball
...
ifndef DEBUG_ARG
DEBUG_ARG = -g
#e.g. -ggdb3 is suppsed to be great
# see: http://www.dirac.org/linux/gdb/02a-Memory_Layout_And_The_Stack.php
endif
...
CFLAGS += $(DEBUG_ARG) -O$(OPT) -funsigned-char -funsigned-bitfields -fpack-struct \
			 -fshort-enums 
...
# * /home/meh/_avrProjects/sdramThingZero/21-gittifying/_commonCode_localized/_make/reallyCommon3.mk
# *
